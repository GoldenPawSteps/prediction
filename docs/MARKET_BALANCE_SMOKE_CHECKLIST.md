...
- exchange reserve and cancel refund exactness
...
3. Perform AMM BUY then SELL.
Expected: BUY debits by `trade.totalCost`, SELL credits by `abs(trade.totalCost)`.

4. Place GTC BID (`0.42 × 10`) and then cancel.
Expected: reserve debit is exact and cancel fully restores balance.

5. Execute crossing ASK/BID fill after seller inventory setup.
Expected: fill executes and combined buyer+seller balances remain conserved.
